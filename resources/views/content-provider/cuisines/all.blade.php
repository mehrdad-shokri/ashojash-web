    ...
    <div class="box">
        ...
        <div class="box-body">
            <div class="dataTables_wrapper form-inline ">
                    ...
                    <div class="col-sm-12">
                        <table aria-describedby="users_table" role="grid" id="example1"
                            ...
                            <tr role="row">
                                ...
                                <th aria-label="Rendering engine: activate to sort column descending"
                                    aria-sort="ascending" style="width: 182px;" colspan="1" rowspan="1"
                                    aria-controls="example1" tabindex="0" class="sorting_asc">Motto
                                </th>
                                <th aria-label="Rendering engine: activate to sort column descending"
                                    aria-sort="ascending" style="width: 182px;" colspan="1" rowspan="1"
                                    aria-controls="example1" tabindex="0" class="sorting_asc">Venues
                                </th>
                                <th aria-label="Rendering engine: activate to sort column descending"
                                    aria-sort="ascending" style="width: 182px;" colspan="1" rowspan="1"
                                    aria-controls="example1" tabindex="0" class="sorting_asc">top Venues
                                </th>
                            </tr>
                            </thead>

                            <tbody>
                            @foreach($managedCuisines as $k => $cuisine)
                                <tr class="{{$k %2==0 ? "odd" : "even"}} clickable-row" role="row"
                                    data-href="{{action('Admin\CuisinesController@show',$cuisine->id)}}">
                                    <td>{{$cuisine->name}}</td>
                                    <td>{{$cuisine->motto}}</td>
                                    <td>{{$cuisine->venues()->count()}}</td>
                                    <td>{{$cuisine->topVenues()->count()}}</td>
                                </tr>
                            @endforeach
                            </tbody>
                ...
                @include('admin.partials.tablefooter',['elements'=>$managedCuisines])

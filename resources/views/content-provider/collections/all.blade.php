    ...
    <div class="box">
        ...
        <div class="box-body">
            <div class="dataTables_wrapper form-inline ">
                        ...
                        <table aria-describedby="users_table" role="grid" id="example1"
                            ...
                            @foreach($collections as $k => $collection)
                                <tr class="{{$k %2==0 ? "odd" : "even"}} clickable-row" role="row"
                                    data-href="{{action('Admin\CollectionsController@show',$collection->slug)}}">
                                    <td>{{$collection->name}}</td>
                                    <td>{{$collection->description}}</td>
                                    <td>{{jDateTime::dateCarbon($collection->created_at)}}</td>
                ...
                @include('admin.partials.tablefooter',['elements'=>$collections])

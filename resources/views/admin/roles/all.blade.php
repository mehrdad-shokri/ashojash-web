    ...
    <div class="box">
        ...
        <div class="box-body">
            <div class="dataTables_wrapper form-inline ">
                    ...
                    <div class="col-sm-12">
                        <table aria-describedby="users_table" role="grid" id="example1"
                            ...
                            <tr role="row">
                                ...
                                <th aria-label="Rendering engine: activate to sort column descending"
                                    aria-sort="ascending" style="width: 182px;" colspan="1" rowspan="1"
                                    aria-controls="example1" tabindex="0" class="sorting_asc">Description
                                </th>
                            ...
                            @foreach($roles as $k => $role)
                                <tr class="{{$k %2==0 ? "odd" : "even"}} clickable-row" role="row"
                                    data-href="{{action('Admin\RolesController@show',$role->id)}}">
                                    <td>{{$role->name}}</td>
                                    <td>{{$role->display_name}}</td>
                                    <td>{{$role->description}}</td>
                                    <td>{{jDateTime::dateCarbon($role->created_at)}}</td>
                ...
                @include('admin.partials.tablefooter',['elements'=>$roles])

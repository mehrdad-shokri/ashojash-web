...
{
//	run("cd {{release_path}}/bootstrap && mkdir cache");
...
{
	$output = run('if [ -d {{deploy_path}} ] && [ -d {{deploy_path}}/current ]; then sudo php {{deploy_path}}/current/artisan up; fi');
	writeln('<info>' . $output . '</info>');
})->desc('Disable maintenance mode');

task('deploy:down', function ()
{
	$output = run('if [ -d {{deploy_path}} ] && [ -d {{deploy_path}}/current ]; then sudo php {{deploy_path}}/current/artisan down; fi');
	writeln('<error>' . $output . '</error>');
})->desc('Enable maintenance mode');*/
task("db:migrate", function ()
{
	$output = run('if [ -d {{deploy_path}} ] && [ -d {{deploy_path}}/current ]; then sudo php {{deploy_path}}/current/artisan migrate --force; fi');
	writeln('<info>' . $output . '</info>');
...
})->desc("Copying .env.production to .env");
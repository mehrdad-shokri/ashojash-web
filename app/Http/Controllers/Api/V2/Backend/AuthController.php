...
use App\Api\Transformer\PermissionTransformer;
...
class AuthController extends BaseController
{
    ...
    public function postLogin(Request $request)
    {
        ...
        if (Auth::once([$field => $request->get("login"), "password" => $request->get('password'), "email_token" => null, "ban" => 0], $request->get('remember') == true)) {
            $user = Auth::user();
            if ($user->can('see-admin-panel')) {
                $token = JWTAuth::fromUser($user);
                return $this->item(new Token($token), new TokenTransformer());
            } else {
                $this->response->errorBadRequest(trans('auth.sure_at_right_place'));
            }
        }
    ...
    public function refreshToken(Request $request)
    {
        ...
        try {
            ...
            return $this->response->errorBadRequest("The token has been blacklisted.");
